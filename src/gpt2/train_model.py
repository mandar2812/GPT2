...
from src.gpt2.data import Dataset, TokenizedCorpus, Vocab
...
class GPT2TrainingSpec(TrainingSpec):
    ...
    def prepare_datasets(self) -> Tuple[Dataset, Dataset]:
        train_dataset = TokenizedCorpus(
            corpus_path=self.train_corpus, vocab=self.vocab, seq_len=self.seq_len
        )
        eval_dataset = TokenizedCorpus(
            corpus_path=self.eval_corpus, vocab=self.vocab, seq_len=self.seq_len
        )
