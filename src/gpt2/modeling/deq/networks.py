...
from src.gpt2.modeling.deq.blocks import (
    LinearPcDEQ1Block,
    LinearPcDEQ2Block,
    ConvPcDEQ1Block,
    ConvPcDEQ2Block,
    PcTransformerDEQBlock
)
...
class PcDEQTransformer(nn.Module):
    def __init__(self, d_model, nhead, hidden, vocab_size, max_iter=10):
        ...
        self.blocks = [self.transformer_block]

    def forward(self, input_ids, src_mask=None):
        x = self.embedding(input_ids)  # shape (seq_len, batch, d_model)
        z0 = torch.zeros_like(x).relu() + 1e-3
        z_star = self.layer(z0, x, src_mask=src_mask)
