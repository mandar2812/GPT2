...
def _plot_entire_metrics_graph(
...
def _plot_log_scale_metrics_graph(
    train_steps: List[int],
    train_losses: List[float],
    eval_steps: List[int],
    eval_losses: List[float],
...
def _plot_stretched_metrics_graph(
    train_steps: List[int],
    train_losses: List[float],
    eval_steps: List[int],
    eval_losses: List[float],
    ...
    min_loss = min(
        train_losses[-target_range_train:] + eval_losses[-target_range_eval:]
    )
    max_loss = max(
        train_losses[-target_range_train:] + eval_losses[-target_range_eval:]
    )
...
def _plot_highlight_metrics_graph(
    train_steps: List[int],
    train_losses: List[float],
    eval_steps: List[int],
    eval_losses: List[float],
    ...
    for loss in eval_losses[1:]:
        smoothed_eval_losses.append(beta * loss + (1 - beta) * smoothed_eval_losses[-1])

    target_range_train = len(train_steps) * 3 // 10
    target_range_eval = len(eval_steps) * 3 // 10

    plt.plot(
        train_steps[-target_range_train:],
...
def visualize_recorded_metrics(args: argparse.Namespace):
    ...
    plt.subplot(221)
    _plot_entire_metrics_graph(train_steps, train_losses, eval_steps, eval_losses)

    plt.subplot(222)
    _plot_log_scale_metrics_graph(train_steps, train_losses, eval_steps, eval_losses)

    plt.subplot(223)
    _plot_stretched_metrics_graph(train_steps, train_losses, eval_steps, eval_losses)

    plt.subplot(224)
    _plot_highlight_metrics_graph(train_steps, train_losses, eval_steps, eval_losses)
...
def add_subparser(subparsers: argparse._SubParsersAction):
    ...
    parser.add_argument(
        "--figure", default="figure.png", help="output figure image file path"
    )
    parser.add_argument(
        "--model_path", required=True, help="trained GPT-2 model file path"
    )
    parser.add_argument(
        "--interactive", action="store_true", help="show interactive plot window"
    )
