...
def test_recorder_record():
    ...
    assert recorder.batch_metrics == {
        "metric1": [0, 1, 2],
        "metric2": [1, 5, 9],
        "scope/metric1": [0, 1, 2, 3],
        "scope/metric2": [1, 3, 5, 7],
    }
...
def test_recorder_format():
    ...
    assert recorder.format("{scope_metric1:.1f}:{scope_metric2:.1f}") == "1.5:4.0"
    ...
    assert recorder.format("{scope_metric1:.1f}:{scope_metric2:.1f}") == "8.5:8.0"
